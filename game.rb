class Game
	attr_accessor :pacmans_world, :pacmans_score, :pacmans_number_of_moves

	def initialize pacmans_world
		@pacmans_world = pacmans_world
		@pacmans_score = 0
		@pacmans_number_of_moves = 0
	end

	def pacman_makes_a_move pacmans_next_step
		return if finished?
		@pacmans_number_of_moves =+ 1
	...
	def finished?
		@pacmans_world.each_value{|position_holder| return false if position_holder == :pellet}
		true
	end
